...
@test "print duration every two seconds and both duration and error output power a spinner and then sigil" {
    ...
    [[ "$output" =~ ^"${R}message: ${N}${R}message: /${N}${R}message: "(\([12]s\)\ )?"-${N}${R}message: ("[12]"s) \\${N}${R}message: ("[234]"s) |${N}${R}message: ("[45]"s) /${N}${R}message: ("[56]"s) -${N}${R}message: ("[56]"s) \\${N}${R}message: OK ("[67]"s)${N}"$ ]] || dump_output
}
