...
@test "single-line error from the command is suppressed because it falls within the initial delay" {
    run -0 invocationNotification --to overlay --message 'message: ' --initial-delay 1001ms --timespan 1s --inline-stderr --command "$SINGLE_LINE_COMMAND"
...
@test "two-line error from the command appends second line because it falls just within the initial delay" {
    run -0 invocationNotification --to overlay --message 'message: ' --initial-delay 1s --timespan 1s --inline-stderr --command 'seq 1 2 >&2'
    assert_control_output "${R}message: 2${N}"
}

@test "multi-line error from the command appends third and then every second line" {
    run -0 invocationNotification --to overlay --message 'message: ' --initial-delay 1250ms --timespan 1s --inline-stderr --command 'seq 1 9 >&2'
...
@test "large initial delay suppresses all output" {
    run -0 invocationNotification --to overlay --message 'message: ' --initial-delay 4501ms --timespan 1s --inline-stderr --command 'seq 1 9 >&2'
    assert_control_output ''
}

@test "large initial delay suppresses all but the final line" {
    run -0 invocationNotification --to overlay --message 'message: ' --initial-delay 4500ms --timespan 1s --inline-stderr --command 'seq 1 9 >&2'
    assert_control_output "${R}message: 9${N}"
}

@test "large initial delay suppresses all but the second-to-final line" {
    run -0 invocationNotification --to overlay --message 'message: ' --initial-delay 4000ms --timespan 1s --inline-stderr --command 'seq 1 9 >&2'
