...
@test "first print duration every two seconds and error output inline and then sigil" {
    ...
    [ $status -eq 0 ]
    [[ "$output" =~ ^"${R}message: ${N}${R}message: (2s) ${N}${R}message: (4s) ${N}${R}message: ("[45]"s) fourth${N}${R}message: ("[56]"s) fifth${N}"("${R}message: (6s) ${N}")?"${R}message: OK ("[67]"s)${N}"$ ]] || echo "$output" | trcontrols | failThis prefix \# >&3
}
