...
@test "--update-message is not supported with --initial --clear --finish" {
    for c in --initial --clear --finish
    do
	run durationMessage "$c" --id ID --update-message whatever
	[ $status -eq 2 ]
	[ "${lines[0]}" = 'ERROR: --update-message can only be used with --update.' ]
	[ "${lines[2]%% *}" = 'Usage:' ]
    done
}
...
@test "--message is not supported with --clear" {
    ...
    [ $status -eq 2 ]
    [ "${lines[0]}" = 'ERROR: --message cannot be used with --clear.' ]
    [ "${lines[2]%% *}" = 'Usage:' ]
}
