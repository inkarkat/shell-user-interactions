...
@test "print duration every two seconds and error output powers a spinner and then sigil" {
    ...
    [ $status -eq 0 ]
    [ "${output//[34]/X}" = "message: /-\\${SAVE_CURSOR_POSITION}(Xs) third |${RESTORE_CURSOR_POSITION}${ERASE_TO_END}(Xs) fourth |${RESTORE_CURSOR_POSITION}${ERASE_TO_END}(Xs) fifth |${RESTORE_CURSOR_POSITION}${ERASE_TO_END}(Xs) |${RESTORE_CURSOR_POSITION}${ERASE_TO_END}OK (7s)" ]
}
