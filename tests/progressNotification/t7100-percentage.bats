...
@test "append percentage of reports to 100%" {
    runWithInput $'first\nsecond\nthird' progressNotification --to overlay --timespan 0 --percentage 3
    [ $status -eq 0 ]
    [ "$output" = "${R}first (33%)${N}${R}second (66%)${N}${R}third (100%)${N}${C}" ]
}

@test "append percentage of reports to 3%" {
    runWithInput $'first\nsecond\nthird' progressNotification --to overlay --timespan 0 --percentage 100
    [ $status -eq 0 ]
    [ "$output" = "${R}first (1%)${N}${R}second (2%)${N}${R}third (3%)${N}${C}" ]
}

@test "append percentage of reports exceeding 100%" {
    runWithInput $'first\nsecond\nthird' progressNotification --to overlay --timespan 0 --percentage 2
    [ $status -eq 0 ]
...
@test "appended percentage when there is an initial delay includes suppressed reports" {
    runWithInput $'first\nsecond\nthird' progressNotification --to overlay --initial-delay 1001ms --percentage 10
    [ $status -eq 0 ]
    [ "$output" = "${R}third (30%)${N}${C}" ]
}

@test "appended percentage when every other report is suppressed includes suppressed reports" {
    runWithInput $'first\nsecond\nthird\nfourth\nfifth\nsixth\nseventh\neighth' progressNotification --to overlay --timespan 1000ms --percentage 10
    [ $status -eq 0 ]
