...
@test "first print duration every two seconds and error output inline and then sigil" {
    ...
    [ $status -eq 0 ]
    [[ "$output" =~ ^"message: ${SAVE_CURSOR_POSITION}(2s) ${RESTORE_CURSOR_POSITION}${ERASE_TO_END}(4s) ${RESTORE_CURSOR_POSITION}${ERASE_TO_END}("[45]"s) fourth${RESTORE_CURSOR_POSITION}${ERASE_TO_END}(6s) fifth${RESTORE_CURSOR_POSITION}${ERASE_TO_END}(6s) ${RESTORE_CURSOR_POSITION}${ERASE_TO_END}OK (7s)"$ ]] || echo "$output" | trcontrols | failThis prefix \# >&3
}
