...
@test "updating with command that returns non-zero exit status returns that status and restores the message afterwards" {
    ...
    run durationMessage --id ID --update --inline-always --command '(exit 42)'
...
@test "updating with unavailable sink returns 1 and does not invoke the command" {
    ...
    DURATION_MESSAGE_SINK=/dev/full run durationMessage --id ID --update --inline-always "${APPEND_COMMAND_ARGUMENTS[@]}" "$MARKER"
    [ $status -eq 1 ]
    [ "$output" = "" ]
...
@test "updating with unavailable sink returns 1 two times, third updating replaces original message" {
    ...
    DURATION_MESSAGE_SINK=/dev/full run durationMessage --id ID --update --inline-always echo "$COMMAND_OUTPUT"
    [ $status -eq 1 ]
    DURATION_MESSAGE_SINK=/dev/full run durationMessage --id ID --update --inline-always echo "$COMMAND_OUTPUT"
    [ $status -eq 1 ]

    run durationMessage --id ID --update --inline-always echo "$COMMAND_OUTPUT"
