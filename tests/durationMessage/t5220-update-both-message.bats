...
@test "immediately updating with both message and update message updates the message and appends the update message" {
    ...
    run durationMessage --id ID --message "$MESSAGE" --update --inline-always --update-message ', %TIMESTAMP%: testing %COUNT% for %DURATION%'
    [ $status -eq 0 ]
...
@test "updating with both message and update message after some time updates the message and appends the update message" {
    ...
    run durationMessage --id ID --update --inline-always --message "$MESSAGE" --update-message ', %TIMESTAMP%: testing %COUNT% for %DURATION%'
    [ $status -eq 0 ]
    [ "$output" = "${START_MESSAGE//?/}${MESSAGE}${OUTPUT}${CLR}" ]
