...
@test "updating with unavailable sink returns 1 two times, third updating will replace update message and update count" {
    durationMessage --id ID --initial --inline-always
    DURATION_MESSAGE_SINK=/dev/full run durationMessage --id ID --update --inline-always --update-message 'we are not testing yet'
    [ $status -eq 1 ]
    DURATION_MESSAGE_SINK=/dev/full run durationMessage --id ID --update --inline-always --update-message 'we are not testing yet'
    [ $status -eq 1 ]

    run durationMessage --id ID --update --inline-always --update-message 'we are testing with %COUNT%. try'
    [ $status -eq 0 ]
