...
@test "print duration every two seconds and error output inline" {
    ...
    [ $status -eq 0 ]
    [[ "$output" =~ ^"message: ${S}first${RE}"(\(1s\)\ )?"second${RE}("[12]"s) third${RE}("[34]"s) third${RE}("[345]"s) fourth${RE}("[56]"s) fifth${RE}("[56]"s) "$ ]] || echo "$output" | trcontrols | failThis prefix \# >&3
}
...
@test "print duration every two seconds and error output inline and then sigil" {
    ...
    [ $status -eq 0 ]
    [[ "$output" =~ ^"message: ${S}first${RE}"(\(1s\)\ )?"second${RE}("[12]"s) third${RE}("[34]"s) third${RE}("[345]"s) fourth${RE}("[56]"s) fifth${RE}("[56]"s) ${RE}OK ("[567]"s)"$ ]] || echo "$output" | trcontrols | failThis prefix \# >&3
}
