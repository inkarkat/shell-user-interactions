...
@test "print duration when incomplete lines are printed slowly" {
    ...
    [ $status -eq 0 ]
    [[ "$output" =~ ^"message: ${S}("[34]"s) ${RE}("[56]"s) ${RE}("[56]"s) first.second.third.fourth${RE}("(9|10|11)"s) first.second.third.fourth${RE}("1[23]"s) first.second.third.fourth${RE}("1[23]"s) ABC"$ ]] || echo "$output" | trcontrols | failThis prefix \# >&3
}
